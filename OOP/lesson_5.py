...
class Cat:
    ...
    def show_name(self):  # прописываем объект, к которому обращаемся
        if hasattr(self, 'name'):  # делаем проверку, есть ли аттрибут в классе
            print(f'Name = {self.name}')  # обращаемся к объекту через Ф строку
        else:
...
# АРГУМЕНТ self В МЕТОДЕ ИСПОЛЬЗУЕТСЯ АВТОМАТИЧЕСКИ ПИТОНОМ