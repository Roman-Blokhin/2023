...
# мы можем не реализовывать отдельно проверки на >= или !=, потому что python сначала проверяет на ==,
# а потом меняет значения местами и сравнивает их с помощью методов __eq__ и __lt__, но не с простыми числами
# достаточно реализаций сравнений __eq__, __lt__, __le__
